...
public class Agent<T extends State<T>> extends PausableThread {
    ...
    static {
        try {
            store = Files.getFileStore(Path.of("C:"));
        } catch (IOException e) {
    ...
    public Agent(T state) {
        ...
        try {
            System.out.println((store.getUsableSpace() >> 30));
        } catch (IOException e) {
        }
    }
    ...
    public T updateState(T state, boolean pause) {
        try {
            ...
            System.out.println("\033[31;1;4m OutOfMemory in Update \033[0m");
        }
        return currentState;
    }
    ...
    public boolean noMemory() {
        return store.getUsableSpace() >> 30 < memSafety;
    }

    protected void loop() {
        if (noMemory()) {
            System.out.println("\033[33;3m No Memory - stalling \033[0m");
            memFlag = true;
        }
        while (noMemory()) if (pausing()) return;
        onSpinWait();
        if (memFlag) {
            System.out.println("\033[33;3m Memory Released - running \033[0m");
            memFlag = false;
        }
        if (stopping()) return;
        if (backlog.isEmpty()) {
            System.out.println("\033[31;1;4m Backlog Exhausted \033[0m");
            Stop();
            return;
        }
