...
public abstract class State<T extends State<T>> extends NodeMap.Node<T> {
    ...
    public static <T extends State<T>> T[] newAlphaBeta() {
        return (T[]) new State[]{MIN_STATE, MAX_STATE};
    }

    private static State<?> artificialState(int fitness) {
        return new State(fitness) {
            @Override
            protected int hashIdentifier() {
                return fitness;
            }

            @Override
            protected int evaluateFitness() {
                return fitness;
            }

            @Override
            public LinkedHashSet<Actionable<?>> getActionables(boolean minMax) {
    ...
    public abstract static class Action<T extends State<T>> implements Function<T, T>, Comparable<Action<T>> {
        ...
        public final T apply() {
            return state.addChild(this.apply(state));
        }

        protected abstract int evaluateFitness();

        public final int fitness() {
            return fitness == null ? fitness = evaluateFitness() : fitness;
        }

        public final int compareTo(Action<T> other) {
            return this.fitness() == other.fitness() ? 1 : other.fitness() - this.fitness();
        }
    }
    ...
    private State(int fitness) {
        this.fitness = fitness;
    }


    public Iterator<T> iterator() {
        return maximize() ? children.iterator() : children.descendingIterator();
    }
    ...
    public final T minMax() {
        return minMax(null, 0);
    } // 'ab=null' works out here as 'ab' is not used if depth < 1

    public final void minMax(T[] ab) {
        minMax(ab, 1);
    }

    public final T minMax(T[] ab, int depth) {
        return minMax(ab, depth, minimize());
    }
    ...
    public final T minMax(T[] ab, int depth, boolean minMax) {
        try {
            depth--;
            if (!children.isEmpty())
                return (minMax ? children.getLast() : children.getFirst()).minMax(ab, depth, minMax); // skip calculations if already processed
    ...
    public final T max(T other) {
        return fitness() > other.fitness() ? (T) this : other;
    }

    public final T min(T other) {
        return fitness() < other.fitness() ? (T) this : other;
    }
    ...
    public boolean isCheck(Color color) { // wtf is this
        return false;
    }
}
